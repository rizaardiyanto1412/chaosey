...
const autoTerrainBtn = document.getElementById("autoTerrainBtn");
const saveLocalBtn = document.getElementById("saveLocalBtn");
const loadLocalBtn = document.getElementById("loadLocalBtn");
const downloadBtn = document.getElementById("downloadBtn");
const importBtn = document.getElementById("importBtn");
const importFileInput = document.getElementById("importFile");
const tilesetQuickPick = document.getElementById("tilesetQuickPick");
...
let currentPaletteRects = [];
let autoDetectedRects = [];
let paletteCells = [];
let autotileMaskToTile = null;
const tilesetImage = new Image();
tilesetImage.decoding = "async";
const TILESET_PRESETS = {
...
function detectBestGridConfig(image) {
    const w = image.width;
    ...
    for (let tileW = 24; tileW <= 256; tileW += 1) {
        for (let tileH = 24; tileH <= 256; tileH += 1) {
            for (let offsetX = 0; offsetX <= 16; offsetX += 1) {
                if ((w - offsetX) <= 0 || (w - offsetX) % tileW !== 0)
                    continue;
                const cols = (w - offsetX) / tileW;
                if (cols < 3 || cols > 16)
                    continue;
                for (let offsetY = 0; offsetY <= 16; offsetY += 1) {
                    if ((h - offsetY) <= 0 || (h - offsetY) % tileH !== 0)
                        continue;
...
function detectLineGridRects(image) {
    if (!image.width || !image.height)
        return [];
    const canvas = document.createElement("canvas");
    canvas.width = image.width;
    ...
    const colLineCandidates = [];
    const rowLineCandidates = [];
    for (let x = 0; x < w; x += 1) {
        if (colDark[x] > h * 0.72)
            colLineCandidates.push(x);
    }
    for (let y = 0; y < h; y += 1) {
        if (rowDark[y] > w * 0.72)
            rowLineCandidates.push(y);
    }
    const group = (arr) => {
        if (arr.length === 0)
            return [];
    ...
    if (colGroups.length < 3 || rowGroups.length < 3)
        return [];
    const rects = [];
...
function buildGridRects(config = atlasConfig()) {
...
function buildAutotileMaskToTileMap(rects) {
    if (rects.length < 16 || !tilesetImage.width || !tilesetImage.height)
        return null;
    const canvas = document.createElement("canvas");
    ...
    const edgeHasGrass = (rect, side) => {
        ...
        for (let dy = 0; dy < rect.sh; dy += 1) {
            for (let dx = 0; dx < rect.sw; dx += 1) {
                const onSide = (side === "N" && dy < sampleDepth && dx >= inset && dx < rect.sw - inset) ||
                ...
                if (isGrass(data[idx], data[idx + 1], data[idx + 2]))
                    grass += 1;
                total += 1;
            }
        }
...
function recomputeAutoTerrainTiles() {
    ...
    for (let row = 0; row < rows; row += 1) {
        for (let col = 0; col < cols; col += 1) {
            ...
            const mask = (isFilled(col, row - 1) ? 1 : 0) |
...
function importJson(file) {
    ...
    reader.onload = () => {
        try {
            applyDocument(JSON.parse(String(reader.result)));
...
function loadTileset(onLoaded) {
    ...
    tilesetImage.onload = () => {
        ...
        updateAutoTerrainUi();
        palettePage = 0;
        setStatus(`Tileset loaded: ${tilesetImage.width}x${tilesetImage.height}, auto=${autoDetectedRects.length}, line-grid=${lineGridRects.length}, mode=${tileMode}.`);
    };
...
[tileWInput, tileHInput, spacingXInput, spacingYInput, offsetXInput, offsetYInput].forEach((el) => el.addEventListener("change", () => {
    currentPaletteRects = buildGridRects();
    selectedTileIndex = 0;
    setTileMode("grid");
}));
